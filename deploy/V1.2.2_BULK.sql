...
CREATE OR REPLACE TABLE bulk.customer
(
ID INT,
...
CREATE OR REPLACE TABLE bulk.foods
(
FOOD_ID INT,
...
CREATE OR REPLACE TABLE bulk.week_1_sales
(
CUSTOMER_ID INT,
FOOD_ID INT
);

CREATE OR REPLACE TABLE bulk.week_2_sales
(
CUSTOMER_ID INT,
FOOD_ID INT
);
...
CREATE OR REPLACE STAGE bulk.customers
...
COPY INTO bulk.customer
FROM @bulk.customers
FILE_FORMAT=bulk.csv
PATTERN='.*Customers.*';


COPY INTO bulk.foods
FROM @bulk.customers
FILE_FORMAT=bulk.csv
PATTERN='.*Foods.*';


COPY INTO bulk.WEEK_1_SALES
FROM @bulk.customers
FILE_FORMAT=bulk.csv
PATTERN='.*Week1_Sales.*';


COPY INTO bulk.WEEK_2_SALES
FROM @bulk.customers
FILE_FORMAT=bulk.csv
PATTERN='.*Week2_Sales.*';
